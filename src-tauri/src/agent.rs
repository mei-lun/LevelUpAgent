...
const LEGACY_SYSTEM_PROMPT: &str = "You are LevelUpAgent, a precise local development agent. Work only inside the selected workspace. Inspect before editing, keep changes focused, explain consequential decisions, and never claim a tool action succeeded until its result is returned. Use tools whenever local evidence is needed.";

// This is a compile-time-only evaluation switch. It is intentionally absent from normal builds,
// so the model cannot discover the treatment arm from the process environment.
pub const EVAL_LEGACY_PROMPT: bool = option_env!("LEVELUP_EVAL_LEGACY_PROMPT").is_some();
...
fn system_prompt_with_omission(request: &AgentTurnRequest, omission: &ContextOmission) -> String {
    ...
    if EVAL_LEGACY_PROMPT {
        legacy_system_prompt_with_omission(request, omission)
    } else {
        crate::harness::compile_system_prompt(request, omission_notice.as_deref()).text
    }
}

fn legacy_system_prompt_with_omission(
    request: &AgentTurnRequest,
    omission: &ContextOmission,
) -> String {
    ...
    {
        Some(workspace) => format!("{LEGACY_SYSTEM_PROMPT}\nSelected workspace: {workspace}"),
        None => format!(
